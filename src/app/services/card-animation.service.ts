...
export class CardAnimationService {
  ...
  observeElement(element: ElementRef, animationClass: string = 'show'): void {
    ...
    const observer = new IntersectionObserver(
      (entries) => { // გადმოცემული მონაცემები, რაც დაკავშირებულია ელემენტის სტატუსთან (ჩნდება/გაქრება ეკრანზე)
        entries.forEach(entry => {
          if (entry.isIntersecting) { // თუ ელემენტი ეკრანზე გამოჩნდა
      ...
      { threshold: 0.4 } // თუ ელემენტის 40%-ზე მეტი გამოჩნდა ეკრანზე, ვთვლით რომ გამოჩნდა
    );
